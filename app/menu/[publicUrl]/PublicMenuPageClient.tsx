...
type PublicMenuPageClientProps = {
  publicUrl?: string;
  initialPayload?: PublicMenuPagePayload;
};
...
export default function PublicMenuPageClient({
  ...
  const publicUrl = routePublicUrl || (params?.publicUrl as string);

  const [restaurantId, setRestaurantId] = useState<string | null>(
    initialPayload?.restaurantId ?? null,
  );
  const [restaurant, setRestaurant] = useState<any>(
    initialPayload?.restaurant ?? null,
  );
  const [menuItems, setMenuItems] = useState<MenuItem[]>(
    initialPayload?.menuItems ?? [],
  );
  ...
  useEffect(() => {
    const loadMenuData = async () => {
      ...
      if (
        initialPayload &&
        initialPayload.publicUrl === publicUrl &&
        initialPayload.restaurantId &&
        Array.isArray(initialPayload.menuItems)
      ) {
        setRestaurantId(initialPayload.restaurantId);
        setRestaurant(initialPayload.restaurant);
